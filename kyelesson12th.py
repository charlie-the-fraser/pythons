    ...
    while flag:
        ...
        try:
           datetime.strptime(user_date, "%d/%m/%Y").date()
            ...
            if int(splitdate[2]) > int(date[2]):
                incorrect = True
            elif int(splitdate[2]) == int(date[2]):
                if int(splitdate[1]) > int(date[1]):
                    incorrect = True
                elif int(splitdate[1]) == int(date[1]):
                    if int(splitdate[0]) > int(date[0]):
                        incorrect = True
            if incorrect == True:
                input("sorry, you did not enter a valid date")
                flag = True
            else:
